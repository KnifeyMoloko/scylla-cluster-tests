...
class ClusterTester(db_stats.TestStatsMixin, unittest.TestCase):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def populate_data_parallel(self, size_in_gb, replication_factor: int = 3, blocking=True, read=False):
        ...
        stress_fixed_params = f" -schema 'replication(factor={replication_factor}) " \
                              f"compaction(strategy=LeveledCompactionStrategy)' " \
                              f"-port jmx=6868 -mode cql3 native -rate threads=200 -col 'size=FIXED(1024) n=FIXED(1)' "
        stress_keys = "n="
        population = " -pop seq="
